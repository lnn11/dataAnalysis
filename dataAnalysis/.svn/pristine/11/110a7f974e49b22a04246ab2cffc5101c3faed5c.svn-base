...
function getActivityData (startTime,endTime,data,activityId) {
	...
	if(startTime > "2017-08-02 00:00:00") {
		$.ajax({
			...
			//优惠券
			$.ajax({
				url:"marketing/newSpringGetCollarRed",
...
//积分奖励值
	...
	$("#2018CountTimes").html(data["2018CountTimes"]);
	...
	$("#2018CountNumber").html(data["2018CountNumber"]);

	//参与数据
	$("#participationDataTimes").html(data["participateInData"]);
	$("#participationDataNumber").html(data["disParticipateInData"]);
	//中奖数据
	$("#prizeDataTimes").html(data["winningData"]);
	$("#prizeDataNumber").html(data["disWinningData"]);
	//抽奖数据
	$("#luckDraw").html(data["luckDraw"]);
	$("#disLuckDraw").html(data["disLuckDraw"]);
}
