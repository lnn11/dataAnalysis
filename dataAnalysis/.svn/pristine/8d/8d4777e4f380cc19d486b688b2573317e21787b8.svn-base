...
<style type="text/css">
	...
	<%@ include file="/WEB-INF/views/aiPinXuan/center/marketing/activityComm.jsp" %>
	<div style="border: solid #CFCFCF; border-width: 1px 1px 1px 1px; margin: 0px 10px 10px 10px; height: 750px;width: 1120px;">
		<div style="margin-top: 20px;margin-left: 10px;">
			<span class="span">数据选择：</span>
			<input type="radio" name="datas" value="1" checked="checked">总数据
			<input type="radio" name="datas" value="2">六折数据
			<input type="radio" name="datas" value="3">五折数据
			<a href="javascript:void(0);" id="export" style="margin-left: 30px;text-decoration: none;">数据导出</a>
			<span style="font-size: 14px; margin-left: 170px;">选择统计周期：</span>
			<input type="text" id="startTime" onclick="WdatePicker({skin:'whyGreen',minDate:'${activity.startTime}',maxDate:'${activity.endTime}'})">
			<span>至</span>
			<input type="text" id="endTime" onclick="WdatePicker({skin:'whyGreen',minDate:'${activity.startTime}',maxDate:'${activity.endTime}'})">
			<a id="query" class="easyui-linkbutton" style="background-color: #e2e2e2">查询</a>
			<a id="queryAll" class="easyui-linkbutton" style="background-color: #e2e2e2">全部</a>
		</div>
	...
	<script type="text/javascript" src="js/aiPinXuan/center/marketing/silkRoad.js"></script>
</body>
</html>