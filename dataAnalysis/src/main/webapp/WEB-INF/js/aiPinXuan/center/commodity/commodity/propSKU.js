...
endDate = $("#endDate").val();
$("#propStartDate").text(startDate);
$("#propEndDate").text(endDate);
$("#propShortName").text($("#propSelect").val());

getProductDate(startDate,endDate,page);
loadProductDataPic();
getSupplier();
...
$("#propTiming").click(function() {
	$("#centerComm").panel({
		href:'aiPinXuan/center/commodity/commodity/propTimingSKU'
	})
})

function tong_propSKU(){
	page = 1;
	startDate =  $("#startDate").val();
	endDate = $("#endDate").val();
	$("#propStartDate").text(startDate);
...
function getProductDate(startDate,endDate,page){
	arrayList = [];
	arrayList = toGetProduct(startDate,endDate,page);
};
...
function toGetProduct(startDate,endDate,page){
	var list = [];
	var pageSize = 10;
	if(page == null){
		page = 1;
	...
	$.ajax({
		type:"post",
		url:'product/getProductProportion',
		data:{"startDate":startDate,"endDate":endDate,"currentPage":page,"pageSize":pageSize,"shortName":$("#propSelect").val()},
		dataType:"json",
		async:false,
...
function loadProductDataPic(){
	...
	if(titleNum.length==0) {
		...
		$("#propSku").append("<td id='color'>"+pname+"</td>");
...
function piePropSku1(array1) {
	...
	var pieOption={
		title : {
	        text: $("#startDate").val()+'至'+$("#endDate").val()+'各品类销量占比',
	        x:'center'
	    },
	    tooltip : {
	        trigger: 'item',
...
function piePropSku2(array2){
	...
	var pieOption={
		title : {
	        text: $("#startDate").val()+'至'+$("#endDate").val()+'各品类销售额占比',
	        x:'center'
	    },
	    tooltip : {
	        trigger: 'item',
